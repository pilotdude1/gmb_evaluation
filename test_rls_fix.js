// Test RLS fix - paste into browser console
...
if (!authKey) {
  ...
  if (!authData.access_token || !authData.user?.id) {
    ...
    // Test account creation with the fixed RLS policies
    const accountData = {
      name: 'Test Account - RLS Fix',
      industry: 'technology',
    ...
    fetch('https://gcgejeliljokhkuvpsxf.supabase.co/rest/v1/crm_accounts', {
      ...
      .then((result) => {
        ...
        if (result[0] && result[0].id) {
          console.log(
            '✅ Account created successfully! RLS fix worked!',
            result[0]
          );
        } else {
          console.log('❌ Account creation still failed:', result);
